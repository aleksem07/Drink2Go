...
const paginationMobileNext = document.querySelector('.arrow-mobile__next');
const paginationMobilePrev = document.querySelector('.arrow-mobile__back');
...
paginationBox.addEventListener('click', (evt) => {
  ...
  if (
    ...
    for (let i = 0; i < paginationPage.length; i++) {
      if (paginationPage[i].classList.contains('pagination__link--active')) {
        paginationPage[i].classList.remove('pagination__link--active');
        paginationPage[i + 1].classList.add('pagination__link--active');
        return;
      }
    }
  } else if (
